...
void read_positions(const char *filename, particle_t *p, u64 n)
{
  ...
  if (file != NULL)
  {
    for (u64 i = 0; i < n; i++)
    {
      if (fscanf(file, "%f %f %f", &p[i].x, &p[i].y, &p[i].z) != 3)
      {
...
//stabl num
	...
	{ 
	  ...
	  for (u64 i = 0; i < n; i++){
	    delta += fabs(p_ref[i].x - p[i].x);
	    delta += fabs(p_ref[i].y - p[i].y);
	    delta += fabs(p_ref[i].z - p[i].z);
	    }
